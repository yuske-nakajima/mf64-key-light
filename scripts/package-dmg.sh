...
# Bundle.module の解決には実行ファイルと同じ階層（Contents/MacOS/）に
# <PackageName>_<Target>.bundle が必要。
cp -R "$RELEASE_DIR/$RESOURCE_BUNDLE" "$APP_DIR/Contents/MacOS/$RESOURCE_BUNDLE"
...
2. 初回起動（無署名アプリの Gatekeeper 回避）
   このアプリは署名されていません。ダブルクリックでは
